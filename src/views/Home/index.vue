    ...
    <div class="banner">
      <el-carousel indicator-position="outside" height="480px">
        <el-carousel-item v-for="item in banner" :key="item.id">
          <!-- 如果有值，则显示 -->
          <!-- 3张图片叠加 -->
          <img v-if="item.picUrl" class="img1" v-lazy="item.picUrl" alt />
          <img v-if="item.picUrl2" class="img2 a" v-lazy="item.picUrl2" alt />
          <img v-if="item.picUrl3" class="img3 b" v-lazy="item.picUrl3" alt />
        </el-carousel-item>
      </el-carousel>
    </div>
    <!-- 多个活动板块v-for -->
      ...
      <section class="w mt30 clearfix" v-if="item.type===2">
        <m-shelf :title="item.name">
          ...
          <div slot="content" class="hot">
            <!-- 将数据传递给子组件 -->
            <mall-goods v-for="o in item.panelContents" :key="o.id" :goods="o"></mall-goods>
          </div>
        </m-shelf>
      </section>
      <section class="w mt30 clearfix" v-if="item.type===3">
        <m-shelf :title="item.name">
          ...
          <div slot="content" class="floors">
            ...
            <!-- 判断类型 -->
            <mall-goods v-for="o in item.panelContents" :key="o.id" :goods="o" v-if="o.type===0"></mall-goods>
          </div>
        </m-shelf>
      </section>
    </div>
  ...
  async created() {
    ...
    try {
      ...
      if (data.code === 200) {
        ...
        //获取轮播图数据
        const item = result.find((item) => item.type === 0);
